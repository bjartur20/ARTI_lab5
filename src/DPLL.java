...
public class DPLL implements InferenceProcedure {
	...
	public boolean isEntailed(KnowledgeBase kb, Sentence alpha) {
		...
		boolean result = !dpll(kbAndNotAlpha, new ArrayList<>(symbols), new Model());
		long elapsed = System.nanoTime() - start;

		// Print results
//		System.out.println("Query        : " + alpha);
		System.out.println("Run-time     : " + (float) elapsed/1000000000);
		System.out.println("Nodes visited: " + this.dpllCalls + "\n");
...
//		while (true) {
//			Pair<PropositionSymbol, Boolean> purePair = findPureSymbol(symbols, clauses, model);
//			if (purePair == null || !purePair.getSecond()) break;
//
//			Set<Clause> clausesCopy = Set.copyOf(clauses);
//
//			for (Clause c: clausesCopy) {
//				for (Literal l: c.getLiterals()) {
//					if (l.getAtomicSentence().equals(purePair.getFirst())) {
//						clauses.remove(c);
//					}
//				}
//			}
...
//		System.out.println("Unknown: " +unknownClauses.toString());
//		System.out.println("Known: " +clauses.toString());

		// P, value <- FIND-PURE-SYMBOL(symbols, clauses, model)
		Pair<PropositionSymbol, Boolean> pureRes = findPureSymbol(symbols, clauses, model);
		// if P is non-null then return DPLL(clauses, symbols - P, model U {P=value})
		if (pureRes != null) {
			PropositionSymbol pureP = pureRes.getFirst();
			Boolean pureVal = pureRes.getSecond();
			List<PropositionSymbol> symbolsCopy = List.copyOf(symbols);
			return callDPLL(clauses, symbolsCopy, model, pureP, pureVal);
		}
